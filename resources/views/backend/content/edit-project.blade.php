            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <small class="text-muted">New file upload না করলে আগেরটা থাকবে।</small>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex gap-2 flex-wrap mb-2" id="existingGrid">
                        ...
                        @php
                            $exLabels = ['1st: At a Glance', '2nd: Gallery Left', '3rd: Gallery Right'];
                            $exLabel  = $exLabels[$i] ?? 'Slider ' . ($i - 2);
                        @endphp
...
<style>
    .img-shaded          { opacity:.25; outline:2px solid red; border-radius:4px; }
...
$(document).ready(function () {
    $('.editor').summernote({
        ...
        resizeable: true,
        callbacks: {
            onKeyup:   function (e) { updateTagInfo(e); },
            onMouseup: function (e) { updateTagInfo(e); },
        }
    });
});
...
function updateTagInfo(e) {
    ...
    let tagName = null, current = node;
    while (current && current !== document) {
        const tag = current.tagName ? current.tagName.toLowerCase() : '';
        if (['h1','h2','h3','h4','h5','h6','p','blockquote','li'].includes(tag)) { tagName = tag.toUpperCase(); break; }
        current = current.parentNode;
    }
    if (!tagName) { $tooltip.hide(); return; }
    $tooltip.html(`&lt;${tagName}&gt;`).css({ left: (e.clientX + 10) + 'px', top: (e.clientY - 30) + 'px' }).show();
...
function previewSingle(input, previewId) {
    ...
    btn.innerHTML = '&times;';
    btn.onclick = () => { input.value = ''; preview.innerHTML = ''; };
...
(function () {
    ...
    function updateExistingLabels() {
        ...
        cards.forEach((card, i) => {
            ...
            lbl.textContent = LABELS[i] ?? `Slider ${i - 2}`;
...
function getNewSlotLabel(newIndex) {
    ...
    if (globalPos < ALL_SLOT_LABELS.length) {
        return { text: ALL_SLOT_LABELS[globalPos], cls: globalPos === 0 ? 'bg-primary' : 'bg-secondary' };
    }
    return { text: `Slider ${globalPos - 2}`, cls: 'bg-dark' };
}
...
// Existing drag-reorder  new labels update 
...
if (existingGridEl) {
    existingGridEl.addEventListener('drop', () => {
        setTimeout(() => { if (newFiles.length > 0) renderNewMulti(); }, 50);
    });
}
...
function renderNewMulti() {
    ...
    newFiles.forEach((item, i) => {
        ...
        // ── label badge — existing count  slot  ──
        ...
        rem.onclick = e => { e.stopPropagation(); removeNewFile(i); };
